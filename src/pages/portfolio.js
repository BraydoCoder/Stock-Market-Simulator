...
function render() {
  ...
  container.innerHTML = `
    <div class="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        ${card(`P&L (${pct(gainPct, false)})`, `${totalGain >= 0 ? '+' : ''}${pc(totalGain)}`, gainClass(totalGain))}
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
          ...
          <div class="bg-surface border border-border rounded-2xl p-5">
            <h2 class="font-semibold text-text-primary mb-4">Allocation</h2>
            ${portVal > 0
  ...
  setTimeout(() => {
    buildDoughnut(allocation, portVal)
...
const SECTOR_COLORS = ['#00D4AA','#6366F1','#10B981','#F59E0B','#EF4444','#3B82F6','#8B5CF6','#EC4899']

function buildDoughnut(allocation, portVal) {
  const canvas = document.getElementById('doughnut-chart')
  if (!canvas || portVal <= 0) return
  ...
  doughnutChart = new Chart(canvas, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => ` ${ctx.label}: ${pc(ctx.raw)} (${((ctx.raw / portVal) * 100).toFixed(1)}%)`,
          },
